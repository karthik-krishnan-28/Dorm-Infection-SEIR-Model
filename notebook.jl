...
# ╔═╡ 61673029-6f8d-4be2-b555-5d7454d0cc93
using Agents # bring package into scope
##
# make the space the agents will live in 

# ╔═╡ 7db8b0f0-dbd1-45da-b7be-7d68bc50d35c
begin
	# PARAMS
	transimission_prob = .30
	sick_population = 10
	healthy_population = 200
end

# ╔═╡ bb074d3e-f6ea-4784-8764-0adfd9d99d73
# INITAL

# ╔═╡ 71950188-4a97-4fb7-ab0a-a8aafe5ab707
@agent struct Student(GridAgent{2}) # inherit all properties of `GridAgent{2}`
    health_status::Int = 1 # The student starts healthy 
end

# ╔═╡ 937d30e0-4f68-4443-bb42-a2575316ee55
function student_step!(agent, model)
    ...
    for neighbor in nearby_agents(agent, model)
		if neighbor.health_status == 2
			health_prop = rand()
			health_prop
			if health_prop < transimission_prob
				agent.health_status = 2 # change current agent to sick
			end
		end 
    end
...
# ╔═╡ 14513100-b9d2-4d8a-8a80-df327aa2ec7a
begin
	function initialize(;gridsize = (20, 20))
		...
		for n in 1:healthy_population
		    add_agent_single!(model; health_status=1)
		end
		for i in 1:sick_population
		    add_agent_single!(model; health_status=2)
		end
	    return model
	end
end

# ╔═╡ a9a162c8-7d48-46a3-af4b-29ed03574f4b
# ╠═╡ disabled = true
#=╠═╡
begin
	...
	adata = [(:health_status, mean), (xpos, mean)]
	adf, mdf = run!(student_model, 10; adata)
	adf # a Julia `DataFrame`
end
  ╠═╡ =#

# ╔═╡ a89806f4-f9bb-4646-ba32-f9d9a58de01e
...
# ╔═╡ bf7c5275-6145-4592-87a1-f766e127246d
...
# ╔═╡ 322c2280-726f-4216-b71c-1eaa067a5cb5
begin
	using CairoMakie # choosing a plotting backend
	student_model = initialize()
	figure_1, _ = abmplot(student_model; agent_color = groupcolor, agent_marker = groupmarker, agent_size = 10)
	figure_1
end


# ╔═╡ a89b9aae-a065-46ca-886e-0d85a7450865
begin
	step!(student_model)
	figure_2, _ = abmplot(student_model; agent_color = groupcolor, agent_marker = groupmarker, agent_size = 10)
	figure_2
end

# ╔═╡ 4959ef75-6c03-462d-923a-204748b388b2
# ╠═╡ disabled = true
#=╠═╡
begin
	abmexploration(
...
# ╔═╡ 54d5369a-8b21-4407-a445-b66e854d4bb1
# ╠═╡ disabled = true
#=╠═╡
begin
	student_model = initialize()
...
manifest_format = "2.0"
project_hash = "2c78e6312686361e8c381a60daec4e7965b58ff9"
...
git-tree-sha1 = "dad69b98444e0a9a7748ee1b990fd2f827632420"
...
git-tree-sha1 = "18959f7acdeeeecc6aec72ff53fb7604b252e22d"
...
git-tree-sha1 = "65b2e49e02c90a5e1b8260f2acaa3eb59a6e4fc9"
